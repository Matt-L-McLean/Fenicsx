#--------------------------------------------------
# This script takes inputs for domain length in x
# and y directions, well diameter, and local
# mesh size. This script returns the gmsh.model file
#--------------------------------------------------

import gmsh # Open source FEM mesh generator
from mpi4py import MPI # Message Passing Interface

rank = MPI.COMM_WORLD.rank # Generate mesh on rank 0
...
def Gmsh_model(ms, mw, xc, yc, wc):
    ...
    if rank == 0:
        ...
        # boundary nodes
        ll = gmsh.model.geo.addPoint(-xc/2,-yc/2,0,ms,1)
        lr = gmsh.model.geo.addPoint(xc/2,-yc/2,0,ms,2)
        ur = gmsh.model.geo.addPoint(xc/2,yc/2,0,ms,3)
        ul = gmsh.model.geo.addPoint(-xc/2,yc/2,0,ms,4)
